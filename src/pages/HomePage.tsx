    ...
    return (
      <div className="min-h-[calc(100vh-3.5rem)] flex flex-col">
        ...
        <div className="flex-1 flex flex-col items-center justify-center px-6 pb-24">
          {/* Viewfinder frame */}
          <div className="relative w-full max-w-sm aspect-[3/4] rounded-3xl border-2 border-dashed border-slate-300 bg-gradient-to-b from-slate-100 to-slate-50 flex flex-col items-center justify-center mb-8 overflow-hidden">
            {/* Corner brackets */}
            ...
            <p className="text-slate-500 font-semibold text-lg mb-1">Point at what's broken</p>
            <p className="text-slate-400 text-sm">Take a photo to get started</p>
          </div>

          {/* Shutter button */}
          <button
            onClick={() => photoRef.current?.click()}
            className="w-20 h-20 rounded-full bg-white border-4 border-brand-500 flex items-center justify-center shadow-lg shutter-ring active:scale-90 transition-transform mb-6"
  ...
  return (
    <div className="min-h-[calc(100vh-3.5rem)] flex flex-col px-4 pt-4 pb-24">
      ...
      <div className="grid grid-cols-2 gap-3 mb-4 animate-fade-in">
        {previews.map((url, i) => (
          <div key={i} className="relative rounded-2xl overflow-hidden border border-slate-200 bg-slate-100 aspect-square shadow-sm">
        ...
        <button
          onClick={() => uploadRef.current?.click()}
          className="rounded-2xl border-2 border-dashed border-slate-300 bg-slate-50 aspect-square flex flex-col items-center justify-center text-slate-400 hover:text-slate-500 hover:border-slate-400 transition-colors"
        >
      ...
      <div className="mb-4 animate-slide-up">
        <textarea
          ...
          className="w-full px-4 py-3 bg-white border border-slate-200 rounded-2xl text-sm text-slate-800 placeholder:text-slate-400 focus:border-brand-500 focus:ring-2 focus:ring-brand-200/50 outline-none resize-none shadow-sm"
          rows={2}
        />
      </div>
      ...
      <div className="flex gap-3 animate-slide-up">
        <button
          ...
          className="flex-1 px-5 py-3.5 bg-white border border-slate-200 text-slate-600 font-semibold rounded-2xl transition-all active:scale-[0.97] shadow-sm"
        >
          Clear
        </button>
        <button
